#!/usr/bin/env python3
"""
TRUE EXPONENTIAL V6: Adaptive Attempts Based on Ceiling Effect

Key insight from V4 analysis:
- Baseline < 13: 100% positive (easy to improve)
- Baseline ≥ 13: 33% positive (ceiling effect)

Solution: ADAPTIVE ATTEMPTS
- Low baseline (< 12): 3 attempts (easy wins)
- Medium baseline (12-13): 5 attempts
- High baseline (≥ 13): 7 attempts (need more to find improvement)

Also: 3 evals (median) for stability
"""
...
def get_attempts(score: int) -> int:
    """Adaptive attempts based on ceiling effect."""
    if score < 12:
        return 3   # Easy to improve
    elif score < 13:
        return 5   # Medium difficulty
    else:
        return 7   # Near ceiling, need more attempts

def run():
    print("="*70)
    print("TRUE EXPONENTIAL V6: ADAPTIVE ATTEMPTS FOR CEILING EFFECT")
    print("="*70)
    print("\nAdaptive: <12 → 3 attempts | 12-13 → 5 attempts | ≥13 → 7 attempts\n")
    ...
    for cycle in range(1, 13):
        ...
        # Adaptive attempts based on score
        n_attempts = get_attempts(current)
        print(f"\n[{n_attempts} improvement attempts (adaptive)...]")
    ...
    low_base = [h for h in history if h["before"] < 13]
    high_base = [h for h in history if h["before"] >= 13]

    if low_base:
        low_pos = sum(1 for h in low_base if h["positive"])
        print(f"\nBaseline < 13: {low_pos}/{len(low_base)} positive")
    if high_base:
        high_pos = sum(1 for h in high_base if h["positive"])
        print(f"Baseline ≥ 13: {high_pos}/{len(high_base)} positive")
    ...
    with open("true_exponential_v6_results.json", "w") as f:
